...
# Now will be used in KasumiC too....
...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:  # pylint:disable=E0602
        ...
        msg = None
        message_to_reply = (
            f"__ Master Sedang AFK!! (BOT BY @KasumiC) #AFK since__ `{total_afk_time}`"
            + f"\n\n_\n**ALASAN**: {alasan}"
            if reason
            else f"**Hey!!**\n`Master Sedang AFK!! (BOT BY @KasumiC  #AFK`\n\n__Sejak__>>> {total_afk_time}"
        )
...
async def _(event):
    ...
    if not USER_AFK:  # pylint:disable=E0602
        ...
        if reason:
            ...
            await borg.send_message(event.chat_id, f"**Saya akan pergi afk!**")
        await asyncio.sleep(2)
        await event.delete()
        try:
            await borg.send_message(  # pylint:disable=E0602
